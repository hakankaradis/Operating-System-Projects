...
// following interfaces "should be only" for internal use by above functions 
//(can't hide them in C, need to use C++ for that and
...
double get_CPU_time_diff()
 ...
 {int passed;DWORD my_PID;//int junk;
	...
	my_PID=GetCurrentProcessId(); /* get the true Process Handle with all 
privileges */
	...
	passed=GetProcessTimes(my_Phandle, 
		 lpCreationTime, lpExitTime, lpKernelTime, lpUserTime);// all 
outputs are FILETIME structures
	/* next line dies, so don't use it */
	//passed=QueryProcessCycleTime(my_Phandle, __out pcycles1); // this call 
gets a "write to 0" failure
	if (!passed) {/*cout<<"failed with code="<<GetLastError(); */return -1;}
	// following 2 lines are for debugging
	//cout<<"USER:highpart="<<UserTime.dwHighDateTime<<"   
lowpart="<<UserTime.dwLowDateTime<<endl;
	//cout<<"KRNL:highpart="<<KernelTime.dwHighDateTime<<"   
lowpart="<<KernelTime.dwLowDateTime<<endl;
...
---------------------- */
	...
	*/
	...
	//lpsystime= &systime;
	//FileTimeToSystemTime(lpKernelTime,lpsystime); /* now can do math on the 
parts */
	//cout<<systime.wMonth<<" "<<systime.wDay<<" "<<systime.wHour<<" 
"<<systime.wMinute<<" "<<systime.wSecond<<" "<<systime.wMilliseconds<<endl;;
	return UserTime.dwLowDateTime;  // give back the milliseconds since process 
started.
...
structure to 
	  obtain relative times. Instead, you should:
			...
			2. Copy the resulting FILETIME structure to a ULARGE_INTEGER 
structure. 
...
structure 
	...
	1. copy the low- and high-order parts of the file time to a LARGE_INTEGER 
structure, 
