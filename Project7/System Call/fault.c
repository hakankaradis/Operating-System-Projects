...
asmlinkage int sys_call_func(int );
...
do_exception(struct pt_regs *regs, unsigned long error_code, int is_protection)
{
	...
	int si_code = SEGV_MAPERR;
        ...
        tsk = current;
        mm = tsk->mm;
	if(countFlag && current->pid == saved_pid){
		printk("countvalue in do exception is %ld\n",countValue);
		countValue++;
	}
...
EXPORT_SYMBOL(sys_call_func);
asmlinkage int sys_call_func(int code){
	printk("sys call code is %d\n countFlag is %d\n countvalue is %ld\n saved pid is %d\n current pid is %d\n", code, countFlag, countValue, saved_pid, current->pid);

	if(code == 0){
		saved_pid = current->pid;
		countFlag=1;
		countValue = 0;
	}
	else if(code == 1){
